...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const cashier = await prisma.user.findFirst({ where: { id, role: ROLES.CASHIER, ownerManagerId: auth.user.id } });
  if (!cashier) {
  ...
  const updated = await prisma.user.update({
    ...
    select: { id: true, email: true, status: true, ownerManagerId: true },
  });
